...
Define ~use-pkg~ macros.

#+BEGIN_SRC emacs-lisp :tangle yes
(cl-defmacro use-pkg (pkg
                      &key
                      disabled
                      setup
                      (feature pkg)
...
#+BEGIN_SRC emacs-lisp
(put 'use-pkg 'lisp-indent-function 'defun)

(defconst use-pkg-font-lock-keywords
  '(("(\\(use-pkg\\)\\_>[ \t']*\\(\\(?:\\sw\\|\\s_\\)+\\)?"
     (1 font-lock-keyword-face)
     (2 font-lock-constant-face nil t))))

(font-lock-add-keywords 'emacs-lisp-mode use-pkg-font-lock-keywords)
#+END_SRC
