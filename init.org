...
*** TODO Write own package helper, works only for =package.el=

#+BEGIN_SRC emacs-lisp :tangle no
;; Support ELPA package
;;; => `cl-defmacro'
(cl-defun my-use-package (pkg
                          &key
                          init          ; before loading feature
                          (feature pkg) ; feature name to load feature
                          config        ; after loading feature
                          )
...
(put 'my-use-package 'lisp-indent-function 'defun)

(defconst my-use-package-font-lock-keywords
  '(("(\\(my-use-package\\)\\_>[ \t']*\\(\\(?:\\sw\\|\\s_\\)+\\)?"
     (1 font-lock-keyword-face)
     (2 font-lock-constant-face nil t))))

(font-lock-add-keywords 'emacs-lisp-mode my-use-package-font-lock-keywords)

(my-use-package 'ace-link
  :init '(ace-link-setup-default))

;;; TODO: some practical example
...
(use-package orglink
  ;; :disabled t                           ; Need to install org from ELPA
  :ensure t
